  ...
  include 'connection.php';
    ...
    if ($error === 0) {
    	...
    	if (in_array($audio_ex_lc, $allowed_exs)) {
            ...
            $sql = "INSERT INTO add_intro  (audio_url,title,topic) 
                   VALUES('$new_audio_name','$title','$topic')";
            $result=mysqli_query($conn, $sql);
			if($result){
				echo '<script>alert("Introdcution added sucessfully")</script>';
			}
			//header("Location: academic-speaking1.php");
            header("Location: academic-speaking1.php");
    	}else {
    		$em = "You can't upload files of this type";
    		header("Location: add-intro.php?error=$em");
    	}
    }
