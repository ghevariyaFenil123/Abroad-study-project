  ...
  include 'connection.php';
    ...
    if ($error === 0) {
    	...
    	if (in_array($audio_ex_lc, $allowed_exs)) {
            ...
            $sql = "INSERT INTO quecard  (audio_url,title,topic) 
                   VALUES('$new_audio_name','$title','$topic')";
            $result=mysqli_query($conn, $sql);
			if($result){
				echo '<script>alert("quecard added sucessfully")</script>';
			}
            //header("Location: academic-speaking2.php");
    	}else {
    		$em = "You can't upload files of this type";
    		header("Location: add-quecard.php?error=$em");
    	}
    }
